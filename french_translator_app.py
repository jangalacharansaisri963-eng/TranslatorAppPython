...
class TerminalTranslator(ctk.CTk):
    def __init__(self):
        ...
        self.to_french = GoogleTranslator(source='en', target='fr')
        self.to_english = GoogleTranslator(source='fr', target='en')
    ...
    def boot_terminal_display(self):
        ...
        boot_msg = (
            ...
            " Supported Targets: 'french' | 'english'\n"
    ...
    def execute_terminal_line(self, event=None):
        ...
        try:
            if target_lang == "french":
                translated_data = self.to_french.translate(payload_phrase)
                ...
                self.append_to_terminal(f" >> SOURCE LANG: FRENCH\n >> TRANSLATION : {translated_data}\n\n")
            else:
                self.append_to_terminal(f" >> TARGET ERROR: Unsupported direction system '{target_lang}'. Use 'french' or 'english'.\n\n")
