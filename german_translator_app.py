...
class GermanTerminalTranslator(BaseTerminalApp):
    def __init__(self):
        # Pass the layout configuration parameters up to the base engine
        super().__init__(
            title_name="Terminal Console - German Core",
            syntax_hints="Syntax Layout:\n   - text (func translate to german)\n   - text (func translate to english)",
            prompt_label="translator@german:~$ "
        )

    def process_command(self, raw_cmd):
        # Extracted pure German translation regex and execution logic
        match_to_german = re.search(r'\s+\(func translate to german\)\s*$', raw_cmd, re.IGNORECASE)
        match_to_english = re.search(r'\s+\(func translate to english\)\s*$', raw_cmd, re.IGNORECASE)

        if match_to_german:
            payload_phrase = raw_cmd[:match_to_german.start()].strip()
            target = "german"
        elif match_to_english:
            payload_phrase = raw_cmd[:match_to_english.start()].strip()
            target = "english"
        else:
            self.terminal_display.insert("end", " >> SYNTAX ERROR: Expecting (func translate to german) or (func translate to english)\n\n")
            return
