...
class TerminalTranslator(ctk.CTk):
    def __init__(self):
        ...
        self.to_german = GoogleTranslator(source='en', target='de')
        self.to_english = GoogleTranslator(source='de', target='en')
    ...
    def boot_terminal_display(self):
        ...
        boot_msg = (
            ...
            " Supported Targets: 'german' | 'english'\n"
    ...
    def execute_terminal_line(self, event=None):
        ...
        try:
            if target_lang == "german":
                translated_data = self.to_german.translate(payload_phrase)
                ...
                self.append_to_terminal(f" >> SOURCE LANG: GERMAN\n >> TRANSLATION : {translated_data}\n\n")
            else:
                self.append_to_terminal(f" >> TARGET ERROR: Unsupported direction system '{target_lang}'. Use 'german' or 'english'.\n\n")
